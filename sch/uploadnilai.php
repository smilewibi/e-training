...
$namafolder="guru/nilai/"; //tempat menyimpan file
$con=include("../Connections/Server_HTC.php");
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	$jenis_file = $_FILES['nama_file']['name'];
	...
	$file = $_POST['File'];
	$id_tgs = $_POST['Id_tgs'];
	$nilai = $namafolder . basename($_FILES['nama_file']['name']);
	if($jenis_file=="file/pdf")
    {
	if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $nilai)) {
			echo "Gambar berhasil dikirim ".$nilai;
  $sql = "UPDATE tugas SET Id_materi='$id_materi', Id_siswa='$id_siswa', Id_guru='$id_guru', `Date`='$date', Judul='$judul', `File`='$file', nilai='$nilai' WHERE Id_tgs='$id_tgs'";
...
if (isset($_GET['Id_tgs'])) {
  $colname_Recordset3 = $_GET['Id_tgs'];
}
mysql_select_db($database_Server_HTC, $Server_HTC);
$query_Recordset3 = sprintf("SELECT `Id_tgs`,tugas.Id_materi,tugas.Id_siswa,tugas.Id_guru,`Date`,`Judul`,`File`,materi.Id_materi,materi.Judul_Materi,materi.Tgl,siswa.id_siswa,siswa.Nama_dpn,siswa.Nm_blkng,siswa.Foto,guru.Id_guru,guru.Nma_dpn,guru.Nma_blkng,guru.Email FROM tugas,materi,siswa,guru WHERE Id_tgs=%s AND guru.Email='$user' AND tugas.Id_materi=materi.Id_materi AND tugas.Id_siswa=siswa.id_siswa AND tugas.Id_guru=guru.Id_guru ORDER BY `Id_tgs` DESC", GetSQLValueString($colname_Recordset3, "int"));
    ...
    <div id="Guest">
      ...
      <table width="100%" border="0">
        <tr>
          <td>Untuk</td>
          <td>:</td>
          <td><?php echo $row_Recordset3['Nma_dpn']; ?><?php echo $row_Recordset3['Nma_blkng']; ?>(<?php echo $row_Recordset3['Email']; ?>)</td>
          <td>Dari</td>
          <td>:</td>
          <td><?php echo $row_Recordset3['Nama_dpn']; ?> <?php echo $row_Recordset3['Nm_blkng']; ?></td>
        ...
        <tr>
          <td colspan="6"><center>
            <h3><?php echo $row_Recordset3['Judul']; ?></h3>
          </center></td>
        </tr>
      </table>
      </p>
      ...
      <table align="center">
        <tr valign="baseline">
          <td nowrap align="right">Id_tgs:</td>
          <td><?php echo $row_Recordset3['Id_tgs']; ?></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Upload nilai:</td>
      ...
      <input type="hidden" name="Id_materi" value="<?php echo htmlentities($row_Recordset3['Id_materi'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="Id_siswa" value="<?php echo htmlentities($row_Recordset3['Id_siswa'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="Id_guru" value="<?php echo htmlentities($row_Recordset3['Id_guru'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="Date" value="<?php echo htmlentities($row_Recordset3['Date'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="Judul" value="<?php echo htmlentities($row_Recordset3['Judul'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="File" value="<?php echo htmlentities($row_Recordset3['File'], ENT_COMPAT, 'utf-8'); ?>">
      <input type="hidden" name="MM_update" value="form1">
      <input type="hidden" name="Id_tgs" value="<?php echo $row_Recordset3['Id_tgs']; ?>">
    </form>
    </div>
    <p>&nbsp;</p>
  </div>
