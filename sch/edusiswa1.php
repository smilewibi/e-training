...
$namafolder="guru/file/"; //tempat menyimpan file
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {
    ...
    $file=$namafolder . basename($_FILES['nama_file']['name']);
		...
		if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $file)) {
			echo "file berhasil dikirim ".$file;
  $sql = "INSERT INTO tugas (Id_tgs, Id_materi, Id_siswa, Id_guru, `Date`, Judul, `File`) VALUES ('$Id_tgs', '$Id_materi', '$Id_siswa', '$Id_guru', '$Date', '$judul', '$file')";
   $res=mysql_query($sql) or die (mysql_error());
		}
  $insertGoTo = "etraining.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
    $insertGoTo .= $_SERVER['QUERY_STRING'];
  }
...
mysql_select_db($database_Server_HTC, $Server_HTC);
$query_Recordset3 = "SELECT id_siswa,Email, siswa.Nama_dpn, siswa.Nm_blkng FROM siswa WHERE Email='$user'";
  ...
  <div id="main">
      ...
      <div id="main">
      	<table width="100%" border="0">
  			<tr>
    			<td>
                	<h3>
                    <?php echo $row_Recordset1['Judul_Materi']; ?> -
					<?php echo $row_Recordset1['Nma_dpn']; ?> 
					<?php echo $row_Recordset1['Nma_blkng']; ?>
                    </h3>
                    <?php echo $row_Recordset1['Tgl']; ?>
                </td>
    			<td>
                	<h4>Upload Tugas</h4>
       					<form method="post" enctype="multipart/form-data" name="form2" action="<?php echo $editFormAction; ?>">
          					<table align="center">
            					<tr valign="baseline">
           						  <td>
                                    <input type="file" id="nama_file" name="nama_file" value="" size="32">
                                    <input type="submit" value="Upload">
                                  </td>
            					</tr>
          					</table>
          					<input type="hidden" name="Id_tgs" value="">
                          <input type="hidden" name="Judul" value="<?php echo $row_Recordset1['Judul_Materi']; ?>">
         				    <input type="hidden" name="Id_materi" value="<?php echo $row_Recordset1['Id_materi']; ?>">
          					<input type="hidden" name="Id_siswa" value="<?php echo $row_Recordset3['id_siswa']; ?>">
	...
	<div id="main">
      <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
        <table align="center">
          <tr valign="baseline">
            <td nowrap align="right" valign="top">Pesan :</td>
        ...
        <input type="hidden" name="guru" value="">
        <input type="hidden" name="siswa" value="<?php echo $row_Recordset3['Nama_dpn']; ?> <?php echo $row_Recordset3['Nm_blkng']; ?>">
      ...
      <table border="0">
        <tr>
          <td><?php if ($pageNum_Recordset4 > 0) { // Show if not first page ?>
            <a href="<?php printf("%s?pageNum_Recordset4=%d%s", $currentPage, 0, $queryString_Recordset4); ?>"><img src="First.gif"></a>
            <?php } // Show if not first page ?></td>
          <td><?php if ($pageNum_Recordset4 > 0) { // Show if not first page ?>
            <a href="<?php printf("%s?pageNum_Recordset4=%d%s", $currentPage, max(0, $pageNum_Recordset4 - 1), $queryString_Recordset4); ?>"><img src="Previous.gif"></a>
            <?php } // Show if not first page ?></td>
          <td><?php if ($pageNum_Recordset4 < $totalPages_Recordset4) { // Show if not last page ?>
            <a href="<?php printf("%s?pageNum_Recordset4=%d%s", $currentPage, min($totalPages_Recordset4, $pageNum_Recordset4 + 1), $queryString_Recordset4); ?>"><img src="Next.gif"></a>
            <?php } // Show if not last page ?></td>
          <td><?php if ($pageNum_Recordset4 < $totalPages_Recordset4) { // Show if not last page ?>
            <a href="<?php printf("%s?pageNum_Recordset4=%d%s", $currentPage, $totalPages_Recordset4, $queryString_Recordset4); ?>"><img src="Last.gif"></a>
            <?php } // Show if not last page ?></td>
        </tr>
      </table>
    </div>
